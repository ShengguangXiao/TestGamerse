...
module.exports = function (callback) {
   ...
   (async () => {
      try {
         const instance = await TestGamerseToken.at("0xDd1CbbAC233D227A997988f71787126bC26FD9D7");
         ...
         console.log(API_KEY);
         if (API_KEY == null) {
            console.log("No walfram api key specified");
            return;
         }

         let waApi = new WolframAlphaAPI(API_KEY);
         let result = await waApi.getFull('roll a dice').then();
         //console.log("result: ", JSON.stringify(result));
         const pods = result["pods"];
         //console.log("pods: ", JSON.stringify(pods));
         const examplePod = pods[4];
         //console.log("example pods: ", examplePod);
         const textResult = examplePod["subpods"][0]["plaintext"];
         //console.log("text result: ", textResult);
         //const diceResult = parseInt(textResult);
         //console.log("dice result: ", diceResult);
         const start = textResult.indexOf(":");
         const end = textResult.indexOf(")");
         //assert.notEqual(start, end);
         if (start != 0 && end != 0 && end > start) {
            const diceResultStr = textResult.slice(start + 1, end);
            const diceResult = parseInt(diceResultStr);
            console.log("diceResult: ", diceResult);

            const testAccount = "0x0BC6817c844391cFEcBA1474a4D1fB3f685F801D";
            
            const currentBalance = await instance.balanceOf(testAccount);
            console.log("Before roll dice balance: ", currentBalance);

            await instance.rollTheDice(diceResult, testAccount);

            const balance = await instance.balanceOf(testAccount);
            console.log("After roll dice balance: ", balance);
         }
      } catch (e) {
         console.error(e);
      }
